...
meta.fiveWMap; //PARAMETER TO BE ADDED IN INPUT TEST
meta.hash5w; //COMPUTE TODO
...
if (sha256(abi.encodePacked(list)) == meta.hash5w) { //check if same 5w already extracted ERROR
...
if ( voters[hash].length > 10){ //DEFINE A TRESHOLD AND TRIGGER TRUSTINESS AND SO ON BASED ON #VOTERS
//consensus achieved
